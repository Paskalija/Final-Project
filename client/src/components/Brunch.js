...
export function Brunch() {
    const [BR, setBrunch] = useState([]);

    function brunch() {
        return axios.get(api.post_get_category_brunch)
        .then(response => response.data)
        .then(data => {
            setBrunch(data.posts)
    })
    ...
    return (
        ...
        <div style={{ background: "	rgb(245, 245, 239)" }}>
            ...
            <Container style={{ marginLeft: "9%", marginRight: "9%", flex: "center", paddingInline: "10%", paddingBottom: "0%" }}>
                <Row>
                    <Row><h3 style={{ color: "rgb(140, 179, 25)", marginBottom: "3%", fontFamily: "Times New Roman", fontWeight: "bold", fontSize: "xx-large", marginLeft: "0%", marginRight: "7%" }}>Brunch</h3></Row>
